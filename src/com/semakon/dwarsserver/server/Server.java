...
public class Server {
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                ClientHandler gch = new ClientHandler(clientSocket.getLocalPort(), // TODO: replace uuid
                        this, clientSocket);
                gch.start();

                lock.writeLock().lock();
                try {
                    clientHandlers.add(gch);
                } finally {
                    lock.writeLock().unlock();
                }
                System.out.println("New client (" + clientSocket.getInputStream() + ") connected on port " + port);
            }
        } catch (IOException e) {
    ...
    public void sendToAll(Message msg) throws SendingMessageException {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if (ch == null) {
                    view.displayError("Empty ClientHandler found, removing...");
                    iter.remove();
     ...
     * @throws SendingMessageException when an error in sending the Message occurs.
     */
    public void sendToClient(ClientHandler recipient, Message msg)
    ...
    /**
     * Gets the ClientHandler whose username corresponds to the given String.
     * @param username the String to check against the ClientHandler's usernames.
    ...
    public void shutdown(int status) {
        try {
            for (ClientHandler gch : clientHandlers) {
                gch.disconnect();
            }
            serverSocket.close();
        } catch (IOException e) {
